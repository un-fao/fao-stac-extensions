...
| --- | --- | --- | --- |
| [`item.json`](item.json) + [`collection.json`](collection.json) | `ASI-D` | `mapset` (Collection) / `map` (Item) | Single-grid global product. Categorical datacube dimensions (`SEASON`, `LCT`), `classification:classes` on the band, `dimension-labels` asset role. The Collection populates every `iso:*` field except `iso:other_constraints`. |
